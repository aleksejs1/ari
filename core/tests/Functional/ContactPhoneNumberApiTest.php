...
class ContactPhoneNumberApiTest extends ApiTestCase
{
    ...
    public function testContactPhoneNumberCRUD(): void
    {
        $client = static::createClient();

        // 1. Create ContactPhoneNumber
        ...
        $phoneNumberIri = $response->toArray()['@id'];

        // 2. GET Item
        $client->request('GET', $phoneNumberIri, [
            'auth_bearer' => $this->token,
        ]);
        ...
        // 3. PUT (Update fully)
        $client->request('PUT', $phoneNumberIri, [
            'auth_bearer' => $this->token,
            'json' => [
                'value' => '+987654321',
                'type' => 'Mobile',
        ...
        $client->request('PATCH', $phoneNumberIri, [
            'auth_bearer' => $this->token,
            'json' => [
                'type' => 'Home',
            ],
        ...
        self::assertResponseIsSuccessful();
        self::assertJsonContains([
            'value' => '+987654321', // unchanged
            'type' => 'Home',
        ]);

        // 5. GET Collection
        $response = $client->request('GET', '/api/contact_phone_numbers', [
            'auth_bearer' => $this->token,
        ...
        $client->request('DELETE', $phoneNumberIri, [
            'auth_bearer' => $this->otherToken,
        ]);
        self::assertResponseStatusCodeSame(404);
    ...
    public function testCannotCreateContactPhoneNumberForOthersContact(): void
    {
        $client = static::createClient();
